...
object Lanternfish {
  ...
  def expectedPopulation(xs: List[String],  days: Int): Long = {
    ...
    val states = 0L.to(8L).groupBy(identity).map{ case (k, v) => if(!input.filterKeys(p => p == k).isEmpty) k -> input(k) else  k -> 0L}
    ...
    def loop(states: Map[Long, Long], days: Int): Long = {
      if (days == 0) states.values.reduce(_ + _)
        ...
        val resets = states(0)
        val updated = 0L.to(8L).groupBy(identity).map{ case (k, v) => k match {
