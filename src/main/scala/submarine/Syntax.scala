...
object Syntax {
  ...
  def calculateErrors(xs: List[String]): Int = {
    val errorsScore = Map(")" -> 3, "]" -> 57, "}" -> 1197, ">" -> 25137)

    xs.map(x => findCorruptedLines(x.split("").toList))
      .filter(a => !errorsScore.keys.find(_ == a.head).isEmpty)
      .map(a => errorsScore(a.head))
      .sum
  }

  def calculateIncomplete(xs: List[String]): Long = {
    val completeScore = Map("(" -> 1, "[" -> 2, "{" -> 3, "<" -> 4)
    ...
    val scores = xs
      .map(x => findCorruptedLines(x.split("").toList))
      .filter(a => !completeScore.keys.find(_ == a.head).isEmpty)
      .map(go(_, 0))
  ...
  def findCorruptedLines(ys: List[String]): List[String] = {
    ...
    def loop(ys: List[String], xs: List[String]): List[String] = {
      ys match {
        case Nil => xs
        case head :: next =>
          if (!opposals.keys.find(_ == head).isEmpty) {
