...
object BinaryDiagnostic {
  ...
  def calculateRate(xs:Map[Int, MapView[String, Int]], mapFunction: MapView[String, Int] => String): Int =
    Integer.parseInt(xs.map { case (k, v) => k -> mapFunction(v)}.toList.sortBy(_._1).map(_._2).mkString, 2)
  ...
  def calculateOxigen(xs: List[String]): Int = {
    def loop(ys: List[String], index: Int): Int = {
      if (ys.size == 1) Integer.parseInt(ys.head,2)
        ...
        loop(ys.filter(c => c(index).toString == occ(index).reduce((a,b) => {
