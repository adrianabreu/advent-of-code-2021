...
object Lava {
      ...
      t._1.zipWithIndex
        .filter(u => {
          neighBourgs
            .map { case (a, b) =>
              if (
                (a + t._2) >= 0 && (a + t._2) < input.size && (b + u._2) >= 0 && (b + u._2) < input(
                  a + t._2
                ).size
              ) input(a + t._2)(b + u._2)
              else 99
  ...
  def calculateBasins(xs: List[String]): Int = {
    ...
    val coords =
      xs.map(_.split("").map(_.toInt).toList).zipWithIndex.map(row => row._1.zipWithIndex)

    def loop(points: Set[(Int, Int)], visited: Set[(Int, Int)], n: Int): Int = {
      val isValid = (a: Int, b: Int) =>
        (a >= 0 && b >= 0 && a < coords.size && b < coords(a).size && visited
          .find(_ == (a, b))
          .isEmpty && (coords(a)(b)._1 < 9))

      val neighbours = points.flatMap { case (x, y) =>
        List(
          (x + 1, y),
          (x - 1, y),
          (x, y + 1),
